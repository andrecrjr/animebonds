...
describe("Main component test", ()=>{
    ...
    it("should search an anime", async ()=>{
        ...
        buttonSearch.click()
        await waitFor(() => new Promise((res) => setTimeout(res, 1))); 
        const inputSearch = await screen.findByPlaceholderText("Search your favorite animes")
        fireEvent.change(inputSearch,{target:{value:animeInput }} )
        await waitFor(() => new Promise((res) => setTimeout(res, 1)));   
        expect(await screen.findByText("Searching for One Piece")).toBeInTheDocument()
        expect(container.firstChild).toMatchSnapshot()
    })
